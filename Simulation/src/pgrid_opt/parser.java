...
public class parser
{
  ...
  public Object[] parseData(String path)
  {
    ...
    float wcost = 0.0F;float scost = 0.0F;float ccurt = 200.0F;
    ...
    if ((token.compareTo(this.rgen) == 1) || (token.compareTo(this.rgen) == 0))
    {
      ...
      for (int i = g.getNNode() - g.getNrgenetarors() - g.getNstorage(); 
            i < g.getNNode() - g.getNstorage(); i++) {
    ...
    g.setNodeList(n);
    token = scanner.next();
    if (((token.compareTo(this.wind) == 1) || (token.compareTo(this.wind) == 0)) && 
      (g.getNrgenetarors() > 0)) {
      for (int i = 0; i < this.ngraph; i++)
      {
    ...
    token = scanner.next();
    if (((token.compareTo(this.solar) == 1) || (token.compareTo(this.solar) == 0)) && 
      (g.getNrgenetarors() > 0)) {
      for (int i = 0; i < this.ngraph; i++)
      {
        Scanner s = new Scanner(scanner.next());
  ...
  {
    boolean m = false;boolean i = false;boolean o = false;
    ...
    for (int j = 0; j < s.length; j++)
    {
      ...
      if ((s[j].compareTo("-i") == 0) && 
        (j + 1 < s.length))
      {
        out[1] = s[(j + 1)];
        i = true;
      }
      if ((s[j].compareTo("-o") == 0) && 
        (j + 1 < s.length))
      {
        out[2] = s[(j + 1)];
        o = true;
